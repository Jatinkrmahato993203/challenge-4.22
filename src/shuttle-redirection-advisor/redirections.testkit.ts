...
import type { Gate, GateId, RiskLevel, RouteId, ShuttleRoute } from "../types/models.js";
import type { ForecastResult } from "../congestion-forecaster/forecast.js";

/**
 * Shared fast-check arbitrary helpers for Shuttle_Redirection_Advisor
 * property tests. Mirrors the score ranges from
 * src/congestion-forecaster/risk-level.ts (Req 3.4 / 4.3) and the risk
 * ordering from src/congestion-forecaster/risk-comparison.ts.
 */
 ...
 * scores[0].score are exercised by generateRedirections /
 * risk-comparison.ts, so the rest of the CongestionScorePoint fields are
 * filled with realistic-but-inert defaults.
 */
...
export function makeRoute(routeId: RouteId, servedGateIds: GateId[]): ShuttleRoute {
  return { routeId, servedGateIds };
}

export function gateId(index: number, prefix = "gate"): GateId {
  return `${prefix}-${index}`;
}

export function routeId(index: number, prefix = "route"): RouteId {
  return `${prefix}-${index}`;
}
